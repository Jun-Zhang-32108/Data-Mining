...
# Output the result
output = open("result_1/"+graphID + ".output","w+")
...
output = open("result_1/"+graphID + "_findMax.output","w+")
output.write('# '+ graphID + " " + str(vertices_n) + " " + str(edges_n) + " " + str(k) + "\n")
for i in range(vertices_n):
...
try:
  ...
  output = open("result_1/"+graphID + "_agg.output","w+")
  output.write('# '+ graphID + " " + str(vertices_n) + " " + str(edges_n) + " " + str(k) + "\n")
  for i in range(vertices_n):
  ...
  mean_dist = np.mean(norm_class_numbers)
  std_d = np.std(norm_class_numbers)
  stat.iloc[3,:] = ['Agg', objective_agg, str(class_numbers_agg), mean_dist, std_d]
  output.close()
  print()
except MemoryError:
...
output = open("result_1/"+graphID + "_MBK.output","w+")
output.write('# '+ graphID + " " + str(vertices_n) + " " + str(edges_n) + " " + str(k) + "\n")
for i in range(vertices_n):
...
mean_dist = np.mean(norm_class_numbers)
std_d = np.std(norm_class_numbers)
stat.iloc[4,:] = ['MiniBatchKmeans', objective_MBK, str(class_numbers_MBK), mean_dist, std_d]
print()
...
objective_spectral = objective(edges_df, clustering.labels_, k, class_numbers)
...
output = open("result_1/"+graphID + "_Spectral.output","w+")
output.write('# '+ graphID + " " + str(vertices_n) + " " + str(edges_n) + " " + str(k) + "\n")
for i in range(vertices_n):
